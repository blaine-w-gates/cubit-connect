...
const TaskEditor = memo(function TaskEditor({ task, onCubit }: TaskEditorProps) {
    ...
    return (
        <div className="p-4 mb-3 mx-2 sm:mx-4 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-xl hover:shadow-md transition-all">
            <div className={`flex gap-4 items-start rounded-lg p-2 transition-all ${activeProcessingId === task.id ? 'bg-purple-50 dark:bg-purple-900/20 animate-pulse' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                        ...
                        >
                            {activeProcessingId === task.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Wand2 className="w-3 h-3" />}
            ...
            {expanded && task.sub_steps && (
                <div className="mt-3 ml-2 sm:ml-4 bg-zinc-50 dark:bg-zinc-950/50 rounded-lg p-3 text-sm border-t border-zinc-100 dark:border-zinc-800 space-y-4 border border-zinc-200/50 dark:border-zinc-700/50">
                    {task.sub_steps.map((step, idx) => (
                        <div key={step.id || idx} className={`space-y-2 rounded-md p-1 transition-all ${activeProcessingId === step.id ? 'bg-purple-50 dark:bg-purple-900/20 shadow-[0_0_15px_-3px_rgba(168,85,247,0.2)]' : ''}`}>
                            <div className="flex items-start justify-between gap-2 group">
                                <div className="flex gap-2 flex-1 items-start">
                                    ...
                                    >
                                        {step.isCompleted && <Check className="w-2.5 h-2.5 text-white dark:text-zinc-900" />}
                                    </button>
                                ...
                                >
                                    {activeProcessingId === step.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Wand2 className="w-3 h-3" />}
                            ...
                            {step.sub_steps && step.sub_steps.length > 0 && (
                                <div className="ml-8 pl-3 border-l-2 border-zinc-200 dark:border-zinc-700 space-y-2 mt-2">
                                    {step.sub_steps.map((micro: CubitStep | string, mIdx: number) => {
                                        ...
                                        return (
                                            <div key={microId} className="text-xs text-zinc-500 dark:text-zinc-500 flex gap-2 items-start group/micro">
                                                <span className="text-zinc-400 dark:text-zinc-500 font-mono mt-0.5 select-none">{String.fromCharCode(97 + mIdx)}.</span>
                                                <div className="flex-1 min-w-0">
                                                    {isObject ? (
                                                        <EditableText
                                                            value={microText}
