...
export default function ResultsFeed() {
    ...
    const handleCubit = useCallback(async (taskId: string, context: string, stepId?: string) => {
        // Access latest state without subscription
        const state = useAppStore.getState();
        const { apiKey, transcript: fullTranscript, tasks: currentTasks, updateTask, addMicroSteps } = state;
        ...
        try {
            ...
            if (stepId) {
                ...
                if (parentTask && parentTask.sub_steps) {
                    ...
                    if (currentIndex !== -1) {
                        ...
                        const getText = (s: unknown) => typeof s === 'string' ? s : (s as { text: string }).text;
            ...
            const rawSteps = await GemininService.generateSubSteps(apiKey, context, fullTranscript || undefined, neighborContext);
    ...
    }, [isOnline]); // Only recreate if network status changes
