...
export default function VideoInput({ videoRef, startProcessing }: VideoInputProps) {
    ...
    const runEngine = useCallback(async (transcriptText: string, videoDuration?: number) => {
        // 0. Concurrency Lock
        ...
        try {
            ...
            const newTasks = await GemininService.analyzeTranscript(apiKey, transcriptText, projectType, videoDuration);
            ...
            if (errString.includes('429') || errString.includes('quota') || errString.includes('403')) {
                setEngineError("You've reached the 20 requests quota. To continue, create a new project and new keys by clicking the link.");
                setShowKeyInput(true);
    ...
    const handleTranscriptParsed = useCallback((text: string) => {
        // Capture Duration if available
        const duration = videoRef.current && !isNaN(videoRef.current.duration) ? videoRef.current.duration : undefined;
