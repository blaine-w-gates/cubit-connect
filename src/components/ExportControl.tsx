...
export default function ExportControl({ onPrint, variant = 'row' }: ExportControlProps) {
    ...
    const checkEmpty = () => {
        if (!tasks || tasks.length === 0) {
            setModalAlert("Start by uploading a video and transcript or pasting text of something you want to learn.");
            return true;
        }
        return false;
    };
        ...
        <>
            ...
            <div className={variant === 'row' ? "flex items-center gap-4" : "flex flex-col gap-1"}>
                ...
                >
                    ...
                    <span className={variant === 'row' ? "hidden sm:inline" : ""}>Markdown</span>
                </button>
            </div>
        </>
    );
