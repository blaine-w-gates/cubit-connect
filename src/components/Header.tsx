...
export default function Header({
    onPrint, setIsSettingsOpen,
    confirmingReset, setConfirmingReset, resetProject,
    mounted, tasksLength
}: HeaderProps) {
        ...
        <header className="sticky top-0 z-50 h-[60px] border-b border-zinc-200 dark:border-stone-800 bg-white/90 dark:bg-stone-900/90 backdrop-blur-sm relative">
            <div className="h-full px-4 md:px-6 flex items-center justify-between">
                ...
                <div className="font-bold text-base md:text-lg tracking-tight flex items-center gap-2 font-serif text-zinc-900 dark:text-white">
                    Cubit Connect
                    <span className="text-xs bg-zinc-900 dark:bg-stone-200 text-white dark:text-stone-900 px-2 py-0.5 rounded-none font-mono hidden min-[360px]:inline-block">Engine</span>
                    {!isOnline && (
                ...
                >
                    <div className="space-y-4">
                        <div className="pb-4 border-b border-zinc-100 dark:border-stone-800">
                            <div className="text-xs font-mono text-zinc-400 dark:text-stone-500 uppercase mb-2">Export</div>
                            <ExportControl onPrint={onPrint} variant="col" />
                        </div>
                        <div>
                            <div className="text-xs font-mono text-zinc-400 dark:text-stone-500 uppercase mb-2">Session</div>
                            {mounted && tasksLength > 0 && (
                                <button
                                    onClick={() => {
                                        if (confirmingReset) resetProject();
                        ...
                        <div className="pt-4 border-t border-zinc-100 dark:border-stone-800">
                            <div className="text-xs font-mono text-zinc-400 dark:text-stone-500 uppercase mb-2">Theme</div>
