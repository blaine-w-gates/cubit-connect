    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-stone-900 border-t border-zinc-200 dark:border-stone-800 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] font-sans transition-colors">
            ...
            >
                ...
                {isOpen ? <ChevronDown className="w-4 h-4 text-zinc-400 dark:text-stone-600" /> : <ChevronUp className="w-4 h-4 text-zinc-400 dark:text-stone-600" />}
            </button>
            ...
            {isOpen && (
                <div className="h-48 border-t border-zinc-100 dark:border-stone-800 bg-zinc-50/50 dark:bg-black/20 p-4 overflow-y-auto font-mono text-xs">
                    {logs.map((log) => (
                        <div key={log.id} className={`${log.message.includes('Complete') || log.message.includes('Success') ? 'text-emerald-700 dark:text-emerald-400' : 'text-zinc-700 dark:text-stone-400'} mb-1 border-l-2 border-zinc-300 dark:border-stone-700 pl-2 flex gap-2`}>
                            <span className="text-zinc-400 dark:text-stone-600 font-bold shrink-0">[{log.timestamp}]</span>
