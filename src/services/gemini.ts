...
export const GemininService = {
    ...
    async retryWithBackoff<T>(fn: () => Promise<T>, retries = 3, delay = 2000): Promise<T> {
        try {
            ...
            const isRetryable = msg.includes('503') || msg.includes('overloaded') || msg.includes('429') || isNetwork;
    ...
    async analyzeTranscript(apiKey: string, transcript: string, mode: 'video' | 'text' = 'video', videoDuration?: number): Promise<TaskItem[]> {
        await this.enforceRateLimit();
        const genAI = new GoogleGenerativeAI(apiKey);
        ...
        try {
            const result = await this.retryWithBackoff(() => this.withTimeout(model.generateContent(prompt)));
            ...
            return tasks.map(t => ({ ...t, id: crypto.randomUUID(), screenshot_base64: "", sub_steps: [] }));
        } catch (error: unknown) {
            const err = error as Error;
    ...
    async generateSubSteps(apiKey: string, instruction: string, transcript?: string, neighborContext?: string): Promise<string[]> {
        await this.enforceRateLimit();
        const genAI = new GoogleGenerativeAI(apiKey);
        const model = genAI.getGenerativeModel({ model: MODEL_NAME, generationConfig: { responseMimeType: "application/json" } });
