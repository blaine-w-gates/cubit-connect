...
export const generateTodoMarkdown = (
  ...
  // Priority Dials section
  if (priorityDials.left || priorityDials.right) {
    md += `## 🎯 Priority Focus\n\n`;
    if (priorityDials.left) md += `**Left Dial:** ${priorityDials.left}\n`;
    if (priorityDials.right) md += `**Right Dial:** ${priorityDials.right}\n`;
    md += `\n---\n\n`;
  }
  ...
  const activeTasks = todoRows.filter((r) => !r.isCompleted);
  const completedTasks = todoRows.filter((r) => r.isCompleted);

  if (activeTasks.length > 0) {
    activeTasks.forEach((row, i) => {
      md += `### ${i + 1}. ${row.task || 'Untitled Task'}\n\n`;
